...
from helpers import create_pairwise_sentence_labels
...
def train_or_evaluate(
    ...
    for iter, data in enumerate(dataloader):
        ...
        textf0, textf1, textf2, textf3, vis_f, acouf, speaker_mask, utterance_mask, label_emotion, label_sentiment = (
            [d.cuda() for d in data[:-1]] if cuda else data[:-1])

        dialogue_lengths, label_emotions, label_sentiments = [], [], []
        for j in range(utterance_mask.size(1)):
            mask = utterance_mask[:, j] == 1
            ...
            label_sentiments.append(label_sentiment[:length, j])
        ...
        logit_emo, logit_sen, logit_sft, extracted_feature = model(
            textf0, textf1, textf2, textf3, vis_f, acouf, utterance_mask, speaker_mask,
            dialogue_lengths)

        prob_emo = F.log_softmax(logit_emo, -1)
        loss_emo = loss_fn_emo(prob_emo, label_emo)
        prob_sen = F.log_softmax(logit_sen, -1)
        loss_sen = loss_fn_sen(prob_sen, label_sen)
        prob_sft = F.log_softmax(logit_sft, -1)
        label_sft = create_pairwise_sentence_labels(shift_win, dialogue_lengths,
                                                label_sen)
        loss_sft = loss_fn_sft(prob_sft, label_sft)
