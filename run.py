...
import time
from helpers import DynamicWeightedLoss
from model import GS_Model
from sklearn.metrics import confusion_matrix, classification_report
from trainer import train_or_evaluate, set_seed
...
parser.add_argument(
    '--dataset',
    default='IEMOCAP',
    help='dataset to train and test.MELD/IEMOCAP/IEMOCAP4',
)
parser.add_argument(
    '--textf_mode',
    default='textf0',
...
def main(local_rank):
    ...
    set_seed()
    model = GS_Model(args, embedding_dims, n_classes_emo)
    ...
    for epoch in range(n_epochs):
        if args.dataset == 'MELD':
            trainset = MELD(MELD_path)
        elif args.dataset == 'IEMOCAP':
            trainset = IEMOCAP(IEMOCAP_path)
        elif args.dataset == 'IEMOCAP4':
            trainset = IEMOCAP4(IEMOCAP4_path)

        setup_samplers(trainset, valid_ratio=0.1, epoch=epoch)

        start_time = time.time()

        train_loss, _, _, train_acc_emo, train_f1_emo, _, _, train_acc_sen, train_f1_sen, train_acc_sft, train_f1_sft, _, _, _ = train_or_evaluate(
            model,
            loss_function_emo,
            loss_function_sen,
            loss_function_shift,
        ...
        valid_loss, _, _, valid_acc_emo, valid_f1_emo, _, _, valid_acc_sen, valid_f1_sen, valid_acc_sft, valid_f1_sft, _, _, _ = train_or_evaluate(
            model,
            loss_function_emo,
            loss_function_sen,
            loss_function_shift,
        ...
        if local_rank == 0:
            test_loss, test_label_emo, test_pred_emo, test_acc_emo, test_f1_emo, test_label_sen, test_pred_sen, test_acc_sen, test_f1_sen, test_acc_sft, test_f1_sft, _, test_initial_feats, test_extracted_feats = train_or_evaluate(
                model,
                loss_function_emo,
                loss_function_sen,
                loss_function_shift,
